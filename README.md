...
The same endpoint serves three server-defined tasks - the holistic review, the SEO assist (`{"task": "seo"}`) and the alt text assist (`{"task": "alt"}`, up to 8 images per call, pictures attached as inline base64 only for vision-capable providers) - each with its own prompt and whitelisting parser; the client can only pick a task, never a prompt. For DeepSeek, reasoning mode is disabled on the request (`thinking: disabled`): V4 models otherwise spend the whole `AI_MAX_TOKENS` budget on hidden reasoning and answer in prose instead of the required JSON.
