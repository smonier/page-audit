...
Jahia jContent UI extension (OSGi/Maven, React 18, Webpack + Module Federation) that adds a **Page audit** action on pages in jContent and Page Builder. The action opens a right-side drawer that loads the page in a same-origin preview iframe and audits it across eight tabs. Every tab leads with **actionable recommendations** (severity chip + what is wrong + how to fix it), and every tab badge means the same thing: the number of issues to review. When an LLM provider is configured, the audit also **assists**: ready-to-paste suggestions (meta descriptions, titles, keywords, headings, CTA labels, alt texts, plain-language rewrites, wording fixes) appear next to the findings they address - always opt-in, always display + copy, never written to the repository.

| Tab | What it does |
|---|---|
| **Accessibility** | axe-core against the full WCAG A / AA / AAA + best-practice rule set. Per-level scorecards, violations grouped by severity with element highlighting, engine version and rules-run transparency, the "needs human review" cases axe cannot decide, and a manual checklist for the WCAG criteria no tool can automate. When AI is configured and the page has images without `alt`, an **AI alt text** section proposes alternative text for each (thumbnail, suggested alt ≤125 chars, Copy, highlight, one-line reason), flags purely decorative images ("use an empty alt"), and is written in the page language. With a vision-capable provider (Anthropic, OpenAI) the model sees a downscaled copy of the picture; DeepSeek is text-only, so there the suggestions are inferred from file names and surrounding text and the UI says so. |
| **SEO** | Title and meta-description length bands, `noindex`/`nofollow` detection, canonical URL, Open Graph / Twitter card completeness with a rendered **social sharing preview card**, JSON-LD presence and validity, `<html lang>` vs audited language, image alt coverage, generic anchor texts. When AI is configured, an **AI suggestions** section proposes ready-to-paste `<title>` tags, meta descriptions (with live character counts), `og:title` / `og:description` for the social card, a focus keyword + supporting terms, heading rewrites (current → suggested, with a reason), and **specific labels for generic calls to action** ("Read more", "Click here", "Submit"… detected on links and buttons, 2-3 alternatives each) - each with a **Copy** button and highlight-in-preview for headings and CTAs. Suggestions are grounded in the page text and written in the **page's language** (they are published content); only the reasons follow the editor's UI language. Nothing is written back: the editor pastes into the SEO properties or the heading content. |
...
| **Readability** | Language-aware scoring: Flesch Reading Ease + Flesch-Kincaid grade (EN), Kandel-Moles adaptation (FR). Sentence/paragraph stats, heading structure checks. When AI is configured and the page has long sentences, an **AI plain-language rewrites** section takes up to 6 of the hardest paragraph sentences and proposes a rewrite in the page language (same facts, shorter sentences, common words) with before/after word counts, highlight of the original, Copy and a one-line reason. |
...
The same endpoint serves four server-defined tasks - the holistic review, the SEO assist (`{"task": "seo"}`, incl. CTA labels), the alt text assist (`{"task": "alt"}`, up to 8 images per call, pictures attached as inline base64 only for vision-capable providers) and the plain-language assist (`{"task": "simplify"}`, up to 6 sentences) - each with its own prompt and whitelisting parser; the client can only pick a task, never a prompt. For DeepSeek, reasoning mode is disabled on the request (`thinking: disabled`): V4 models otherwise spend the whole `AI_MAX_TOKENS` budget on hidden reasoning and answer in prose instead of the required JSON.
